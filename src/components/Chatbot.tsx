    ...
    <div className="fixed bottom-4 right-4 z-50">
      {!isOpen && (
        <button
          ...
          className="bg-secondary-500 text-white p-4 rounded-full shadow-lg hover:bg-primary-500 transition-colors"
        >
      ...
      {isOpen && (
        <div className="bg-white rounded-lg shadow-xl w-96 max-h-[600px] flex flex-col">
          <div className="p-4 bg-secondary-500 text-white rounded-t-lg flex justify-between items-center">
          ...
          <div className="p-4 border-b">
            <h4 className="text-sm font-medium text-gray-700 mb-2">Ask me from the example documents!</h4>
          </div>
          ...
          <div ref={chatContainerRef} className="flex-1 overflow-y-auto p-4 space-y-4">
            {messages.map((msg, index) => (
              <div key={index} className={`flex ${msg.isUser ? 'justify-end' : 'justify-start'}`}>
                <div
                  className={`max-w-[80%] rounded-lg p-3 ${msg.isUser ? 'bg-secondary-500 text-white' : 'bg-gray-100 text-gray-800'
                    }`}
                >
          ...
          <form onSubmit={handleSubmit} className="p-4 border-t">
            <div className="flex space-x-2">
              ...
              <button
                type="submit"
                className="bg-secondary-500 text-white p-2 rounded-lg hover:bg-primary-500 transition-colors"
                disabled={isLoading}
              >
